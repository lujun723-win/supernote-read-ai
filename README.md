...
| 文件 | 适用情况 |
| --- | --- |
| `ReadAssist-v1.12.1-full-ecdict.apk` | 推荐。内置 3,402,564 词的 ECDICT，安装后在设置页一键部署 |
| `ReadAssist-v1.12.1-light.apk` | 体积较小，不带词库，需要自行导入 StarDict |
...
4. 启动悬浮按钮服务。
5. 单击 `AI` 打开对话；在 0.8 秒内双击 `AI` 进入区域截图。
...
```bash
./gradlew testDeepseekUnitTest assembleDeepseek assembleDeepseekBundled
```

生成的 APK 位于：

```text
app/build/outputs/apk/deepseek/ReadAssist-v1.12.1-dictionary-deepseek.apk
app/build/outputs/apk/deepseekBundled/ReadAssist-v1.12.1-dictionary-deepseekBundled.apk
```

通过 ADB 安装：

```bash
adb install -r app/build/outputs/apk/deepseekBundled/ReadAssist-v1.12.1-dictionary-deepseekBundled.apk
```
